	...
	return async dispatch => {
		...
		try {
			...
			const responsePipelines = await fetch(
				`${url}/pipelines?id_pipeline=${item.id_pipeline}`,
				{
					method: 'PATCH',
					headers: {
						Accept: 'application/json',
			...
			const response = await fetch(
				`${url}/customers?id_customer=${dataPipelines[0].id_customer}`,
				{
					method: 'GET',
					headers: {
						Accept: 'application/json',
			...
			dispatch(setFailed(true, 'FAILED_POST_ANSWER', e))
			dispatch(setLoading(false, 'LOADING_POST_ANSWER'))
		}
	}
}
