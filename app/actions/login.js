...
export const login = (email, password) => {
  return async (dispatch) => {
    ...
    await dispatch(setFailed({condition: false, process_on: 'login', message: ''}))
    try{
      ...
      if(data.code === 401 && data.name === 'NotAuthenticated') {
        await dispatch(setFailed({condition: true, process_on: 'login', message: 'Email or password incorrect'}))
      ...
      await dispatch(setFailed({condition: true, process_on: 'login', message: e}))
      await dispatch(setLoading({condition: false, process_on: 'login'}))
    }
  }
}
...
const fetchUserWithEmail = (email, password, accessToken) => {
  return async (dispatch) => {
    await dispatch(setLoading({condition: true, process_on: 'fetch_user_with_email'}))
    try {
      ...
      await dispatch(saveSessionForPersistance({data: data.data[0], accessToken: accessToken}))
      await dispatch(setLoading({condition: false, process_on: 'fetch_user_with_email'}))
    }catch(e) {
      await dispatch(setFailed({condition: true, process_on: 'fetch_user_with_email', message: e}))
      await dispatch(setLoading({condition: false, process_on: 'fetch_user_with_email'}))
    }
  }
}