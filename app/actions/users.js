...
export const fetchUsers = (accessToken) => {
  return async (dispatch) => {
    ...
    try {
      ...
      await dispatch(setLoading({condition: false, process_on: 'fetch_users'}))
    }catch(e) {
      await dispatch(setFailed({condition: true, process_on: 'fetch_users', message: e}))
      await dispatch(setLoading({condition: false, process_on: 'fetch_users'}))
    }
  }
}
...
export const updateUser = (id, data, accessToken) => {
  return async (dispatch) => {
    ...
    try {
      ...
      await dispatch(setLoading({condition: false, process_on: 'update_user'}))
    }catch(e) {
      await dispatch(setFailed({condition: true, process_on: 'update_user', message: e}))
      await dispatch(setLoading({condition: false, process_on: 'update_user'}))
    }
  }
}
