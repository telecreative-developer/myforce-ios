...
import { StyleSheet, Dimensions, View, TouchableOpacity, FlatList, Image, TextInput } from 'react-native'
import { Container,
Content,
Header,
Footer,
...
class Activity extends Component {
  ...
  renderItems = ({item}) => (
    <TouchableOpacity style={styles.card}>
      <HorizontalJoker title={item.name} person={item.pic} description={item.description} avatar={item.avatar} />
    </TouchableOpacity>
  )
    ...
    return (
      <Container>
        ...
        <Header style={styles.header}>
          <Left>
            <TouchableOpacity onPress={() => this.props.setNavigate({link: 'Profile'})}>
            {(this.props.sessionPersistance.data.avatar === '' ? (
              <Thumbnail rounded small source={defaultAvatar} />
            ) : (
              <Thumbnail small rounded source={{uri: this.props.sessionPersistance.data.avatar}} />
            ))}
            </TouchableOpacity>
          </Left>
          <Body>
        ...
        <View style={styles.wrapperView}>
          ...
          <TouchableOpacity style={styles.centerButton} onPress={() => this.setState({isModalVisible: true})}>
