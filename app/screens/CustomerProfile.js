...
import { StyleSheet, Dimensions, View, TouchableOpacity, Image, ImageBackground, FlatList, TouchableHighlight, Alert } from 'react-native'
import { Container,
Content,
Header,
Left,
...
import { fetchPipelines, postPipeline, fetchPipelinesRealtime } from '../actions/pipelines'
...
class CustomerProfile extends Component {
  ...
  componentWillMount() {
    this.props.fetchPipelines(this.props.navigation.state.params.id_customer, this.props.sessionPersistance.accessToken)
    this.props.fetchPipelinesRealtime(this.props.navigation.state.params.id_customer, this.props.sessionPersistance.accessToken)
  }

  async handleSubmitPipeline() {
    if(!isEmpty(this.state.pipeline)) {
      await this.setState({modalNewPipeline: false})
      await this.props.postPipeline({pipeline: this.state.pipeline, id_status: 0, id_customer: this.props.navigation.state.params.id_customer}, this.props.sessionPersistance.accessToken)
      ...
      return (
        <View>
          <View style={styles.addPipeline}>
            <Button full style={styles.addPipelineDirection} onPress={() => this.setState({modalNewPipeline: true})}>
  ...
  renderItemsClose = ({item}) => (
    <View style={styles.customerPipeline}>
      <View style={styles.pipelineContent}>
        ...
        <View>
          <PipelineProgress onPress={() => this.setState({isModalVisible: true})} currentPosition={item.id_status} />
        </View>
      </View>
    </View>
  )
    ...
    return (
      <Container>
          ...
          <Footer>
            <FooterTab>
              <Button onPress = {() => this.setState({modalNewPipeline: false})}>
                <Text note style={styles.modalCancelButton}>Cancel</Text>
              </Button>
          ...
          <View style={styles.modalWrapperAddPipeline}>
            ...
            <View style={styles.formPicDirection}>
              <Form>
                ...
                <Item floatingLabel>
                  <Label>Address</Label>
                  <Input multiline={true} value="JL. Pegangsaan 2, Jakarta Utara" style={styles.picAddress}/>
                </Item>
              </Form>
            </View>
          </Content>
        ...
        <Modal style={styles.modal} isVisible={this.state.isModalVisible}>
          ...
          <View>
            <ImageBackground
             source={{uri: 'https://4.imimg.com/data4/TA/RW/MY-5777330/xerox-photocopier-machine-250x250.jpg'}}
        ...
        <View style={styles.newsWrapper}>
          <View style={styles.newsDirection}>
            ...
            <TouchableOpacity style={styles.iconFlex} onPress={() => navigate('OrderSummary')}>
            ...
            <View style={styles.headerDirectionTitle}>
              <View>
                ...
                <View style={styles.headerDirection}>
                  ...
                  <Text style={styles.data}>Jl. Lorem Ipsum No.13 Sudirman, Jakarta Selatan</Text>
                </View>
            ...
            <Grid style={{display: 'flex', alignItems: 'center'}}>
              <Col>
                <TouchableOpacity onPress={() => this.setState({pipelineTabs: 'active'})}>
                  <H1 style={styles.totalText}>{JSON.stringify(this.props.pipelines.filter(p => p.id_status !== 5).length)}</H1>
              ...
              <Col>
                <TouchableOpacity onPress={() => this.setState({pipelineTabs: 'lose'})}>
                  <H1 style={styles.totalText}>{JSON.stringify(this.props.pipelines.filter(p => p.id_status === 7).length)}</H1>
