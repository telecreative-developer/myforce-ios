...
import { StyleSheet, TouchableOpacity, Dimensions, AsyncStorage, View, Alert } from 'react-native'
import { Container,
Header,
Left,
Body,
...
class EditProfile extends Component {
  ...
  componentWillReceiveProps(props) {
    if(props.loading.condition === false && props.loading.process_on === 'update_user') {
  ...
  renderButton() {
    const { first_name, last_name, username, email, address, phone } = this.state
    if(!isEmpty(first_name) && !isEmpty(last_name) && !isEmpty(username) && !isEmpty(email) && !isEmpty(address) && !isEmpty(phone)) {
      return (
        <Button large block onPress={() => this.handleValidation()}>
          {(this.props.loading.condition === true && this.props.loading.process_on === 'update_user') ? (
  ...
  handleValidation() {
    const { first_name, last_name, username, gender, id_region, email, address, phone } = this.state
    if(!isEmail(email)) {
      Alert.alert('Update failed', 'Please input valid email')
    }else{
      this.props.updateUser(this.props.sessionPersistance.data.id, {first_name, last_name, email, username, gender, id_region, address, phone}, this.props.sessionPersistance.accessToken)
    }
  }
